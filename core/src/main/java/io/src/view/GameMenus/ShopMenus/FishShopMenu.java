...
public class FishShopMenu implements AppMenu {
    ...
    public Result check(Scanner scanner, String cmd) {
        ...
        if((StoreCommands.ShowAllProduct.getMatcher(cmd))!=null) {
            return FishShopMenuController.showAllProducts();
        } else if((StoreCommands.ShowAllAvailableProducts.getMatcher(cmd))!=null) {
            return FishShopMenuController.showAllAvailableProducts();
        } else if((matcher = StoreCommands.PurchaseProducts.getMatcher(cmd))!=null) {
            return FishShopMenuController.PurchaseProduct(matcher);
        } else if(cmd.equalsIgnoreCase("exit")) {
            return FishShopMenuController.ExitShop();
        } else {
