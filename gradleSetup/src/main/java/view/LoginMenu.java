...
import model.Enums.MenusRegexes.LoginMenuRegexes;
...
public class LoginMenu implements AppMenu {
    ...
    public void check(Scanner scanner) {
        ...
        Matcher matcher;
        if ((matcher = LoginMenuRegexes.register.getMatcher(input)).find()) {
            System.out.println(LoginMenuController.manageRegisterUser(matcher));
            String input1 = scanner.nextLine();
            Matcher matcher1;
            if ((matcher1 = LoginMenuRegexes.pickSecurityQuestion.getMatcher(input)).find()&&LoginMenuController.manageRegisterUser(matcher).isSuccess()) {
                System.out.println(LoginMenuController.peakSecurityQuestion(matcher1, matcher));
            }
        } else if ((matcher = LoginMenuRegexes.goMenu.getMatcher(input)).find()) {
            String menu = matcher.group(1);
            System.out.println(LoginMenuController.goToMenu(menu));
        } else if((matcher = LoginMenuRegexes.login.getMatcher(input)).find()) {
            System.out.println(LoginMenuController.manageLoginUser(matcher,false));
        } else if((matcher = LoginMenuRegexes.loginWithStayLoggedOut.getMatcher(input)).find()){
            System.out.println(LoginMenuController.manageLoginUser(matcher,true));
        } else if((matcher = LoginMenuRegexes.forgetPassword.getMatcher(input)).find()) {
            System.out.println(LoginMenuController.manageForgotPassword(matcher));
            Matcher matcher1;
            String input1 = scanner.nextLine();
            if((matcher1 = LoginMenuRegexes.answerSecurityQuestion.getMatcher(input)).find()&&LoginMenuController.manageForgotPassword(matcher).isSuccess()) {
        ...
        } else if((matcher = LoginMenuRegexes.ShowCurrentMenu.getMatcher(input)).find()){
